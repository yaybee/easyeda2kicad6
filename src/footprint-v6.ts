...
import { FootprintHead, IEasyEDAFootprint } from './easyeda-types';
...
export function convertFp(
  ...
  let footprintProp: ISpectraList = [];
  ...
  if (isBoard) {
    ...
    const fp = attrs.package.replace(/[^\w\s\.\(\)\-]/g, 'x');
    return [
      '_LF_',
      [
        'footprint',
...
export function convertFootprintV6(footprint: IEasyEDAFootprint): string {
