...
function kiEffects(
  ...
  const font = parseFloat(fontSize.replace('pt', ''));
  if (isNaN(font)) {
    size = 1.27;
  } else {
    if (font > 7) {
...
function convertImage(args: string[], ConversionState: IConversionState): ISpectraList {
  const [x, y, width, height, rotation, href, id, locked] = args;
  const [imageType, imageString] = href.split(',');
  if (imageType === 'data:image/png;base64') {
    const msg1 = `Info: Found image (${id}): it probably needs to be resized and/or moved manually `;
    reportError(msg1, ConversionState);
    const imgUuid: string = uuid();
    return [
      '_LF_',
      ['image', kiAt(x, y), ['scale', 0.2], ['uuid', imgUuid], '_LF1_', ['data', imageString]],
    ] as ISpectraList;
  } else {
    const msg2 = `Warning: Image (${id}) found is not an embedded png image; image ignored`;
...
function convertSchematicV6ToArray(
  ...
  for (shape of schematic.shape) {
    ...
    if (type === 'B') {
      ...
      const nfresult = convertNetflag(args, knownNetflags, conversionState, inputFileName);
      if (nfresult != null) {
      ...
      junctions.push(...convertImage(args, conversionState));
    } else if (type === 'J') {
      junctions.push(...convertJunction(args));
    } else if (type === 'LIB') {
      let symbolInstance: any = [];
      let symbol: any = [];
      [symbolInstance, symbol] = convertLibrary(
        args.join('~'),
        null,
        conversionState,
        CRCTable,
      ...
      const msg1 = `Warning: unsupported shape ${unsupportedShapes[type]} found in schematics; shape ignored`;
      reportError(msg1, conversionState);
    } else {
      const msg2 = `Warning: unknown shape ${type} found in schematics; shape ignored`;
      reportError(msg2, conversionState);
    }
  }
  for (const nport of schNetPorts) {
    netports.push(...convertNetPort(nport));
  }
  for (const msg3 of conversionState.savedLibMsgs) {
...
export function convertSchematicV6(
  input: IEasyEDASchematicCollectionV6,
  sheet: number,
  inputFileName: string
): string[] {
  ...
  const schematics = encodeObject(
    convertSchematicV6ToArray(schematic, conversionState, inputFileName)
  );
